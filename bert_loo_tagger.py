...
def main():
    ...
    if args.task == "SA":
        full_train_examples = sst_processor.get_train_examples(args.data_dir, args.num_train_samples)
    elif args.task == "NLI":
        full_train_examples = mnli_processor.get_train_examples(args.data_dir, args.num_train_samples)
    if args.task == "SA":
        test_examples = sst_processor.get_dev_examples(args.data_dir)
    elif args.task == "NLI":
        test_examples = hans_processor.get_test_examples(args.data_dir)
    
    # Loading influence result
    influences = pickle.load(open(os.path.join(args.influence_file_dir, "influences_test_" + str(args.test_idx) + ".pkl"), "rb"))
    assert args.num_train_samples == len(influences)
    ...
    # train and test models with different loo data
    loo_logits_list = []
    for loo_i, loo_inf_idx in enumerate(loo_influential_idx_list):
        ...
        if args.do_train:
            ...
            model.train()
#             model.eval() # train with eval mode to ignore the influence of dropout
        ...
        if args.do_test:
            ...
            for input_ids, input_mask, segment_ids, label_ids, guids in tqdm(test_dataloader, desc="Testing"):
                cur_test_idx = guids[0].item()
                if cur_test_idx != args.test_idx:
                    continue
                ...
                loo_logits_list.append(logits)
                logger.info('LOO type ' + str(loo_i) + ': ' + str(logits))

                break
                
    pickle.dump(loo_logits_list, open(os.path.join(args.output_dir, "loo_logits_test_" + str(args.test_idx) + ".pkl"), "wb"))
